...
export const useModelAndTokenBrowser = (
	...
	const [selectedModel, setSelectedModel] = useState(() =>
		loadFromStorage(STORAGE_KEYS.MODEL, ""),
	);
		...
		const loadModels = async () => {
			...
			try {
				...
				const baseModels = new Set(
					modelNames.filter((name) => name.length > 0),
				);
				...
				if (hasChannelData) {
					...
					transformedModels = Array.from(modelChannelLabels.entries()).map(
						([modelName, channelSet]) => {
				...
				if (selectedChannel && transformedModels.length === 0) {
					if (channelErrorRef.current !== selectedChannel) {
						const channelLabel =
							channelLabelMap.get(selectedChannel) ??
